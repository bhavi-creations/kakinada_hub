...
<?php
$id = intval($_GET['id']);
$item = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM trending WHERE id = $id"));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        $uploadPath = '../uploads/home_trending/' . basename($newImage);

        if (move_uploaded_file($tmpName, $uploadPath)) {
            // Delete old image
            $oldImagePath = '../uploads/home_trending/' . $item['image'];
    ...
    $updateQuery = "UPDATE trending SET 
        title='$title', description='$description', link='$link', 
        offer='$offer', type='$type', image='$imageName' 
        WHERE id=$id";

    if (mysqli_query($conn, $updateQuery)) {
        $successMsg = "Trending item updated successfully!";
        $item = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM trending WHERE id = $id")); // refresh data
    } else {
        ...
        <div id="content">
            <div class="container-fluid">
                ...
                <div class="d-sm-flex align-items-center justify-content-between mb-4">
                    <h1 class="h3 mb-0 text-gray-800">Edit Trending Item</h1>
                    <a href="view_home_trending.php" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm">
                        <i class="fa-regular fa-eye"></i> View Trending Lists
                    </a>
                </div>
                    ...
                    <form method="POST" enctype="multipart/form-data">
                        <div class="row">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <img src="../uploads/home_trending/<?= $item['image'] ?>" width="150" class="img-thumbnail">
                            </div>
                        ...
                        <a href="view_home_trending.php" class="btn btn-secondary mt-3">Cancel</a>
                    </form>
                </div>
            </div>
        </div>
